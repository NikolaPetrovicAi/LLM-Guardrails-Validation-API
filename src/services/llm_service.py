...
class ViralContentService:
    ...
    async def _run_llm_generation(
        ...
        """
        Helper to run the LLM generation and logging logic for a given prompt definition.
        """
        ...
        if self.langfuse:
            try:
                ...
                print(f"📡 Sending {type_label} data to Langfuse (Trace ID: {request_id})...")
                ...
                logger.error(f"❌ Langfuse logging failed: {str(lf_err)}", exc_info=True)
                print(f"❌ Langfuse error: {str(lf_err)}")

        return response

    async def generate_viral_script(
        self, request: ScriptRequest, prompt_id: str = "tiktok_script_gen", version: str | None = "1.0.0"
    ) -> ViralScriptResponse:
        """
        Generates a viral script with hybrid caching, shadow deployment, and APO support.
        """
        ...
        if prompt_def.shadow_version:
            try:
                shadow_prompt_def = self.prompt_manager.get_prompt(prompt_id, prompt_def.shadow_version)
            except Exception as e:
                logger.warning(f"Could not load shadow version {prompt_def.shadow_version}: {e}")

        if shadow_prompt_def:
            # Create a separate ID for Shadow in Langfuse to avoid collisions
            shadow_trace_id = uuid.uuid4().hex
            logger.info(f"🚀 Shadow Deployment ACTIVE: Triggering {shadow_prompt_def.version} in background (Trace: {shadow_trace_id})")
            
            # Run shadow version asynchronously with a small initial delay
            async def run_shadow_with_delay():
                await asyncio.sleep(0.5) 
                await self._run_llm_generation(request, shadow_prompt_def, shadow_trace_id, is_shadow=True)
            
            asyncio.create_task(run_shadow_with_delay())

        # Step 5: Main LLM Generation
        try:
            response = await self._run_llm_generation(request, prompt_def, request_id, is_shadow=False)

            # Step 6: Automated Prompt Optimization (APO)
            if self.optimizer:
                # Run APO asynchronously so we don't block the user
                asyncio.create_task(
                    self.optimizer.critique_and_suggest(prompt_def, request.model_dump(), response)
                )

            # Step 7: Store in both caches
            resp_json = response.model_dump_json()
            self.cache.set(cache_key, resp_json, expire=self.cache_expire)
            self.semantic_cache.set(masked_text_prod, resp_json, expire=self.cache_expire)
    ...
    async def stream_viral_script(
        self, request: ScriptRequest, prompt_id: str = "tiktok_script_gen", version: str | None = None
