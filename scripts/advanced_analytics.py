...
def run_analytics(num_examples: int = 5, use_random: bool = False, version: str = None):
    ...
    header_text = (
        "[bold cyan]VIRAL ENGINE[/bold cyan] • "
        "[white]Advanced Engineering Analytics[/white]"
    )
    if version:
    ...
    header = Panel.fit(header_text, border_style="cyan", box=box.ROUNDED)
    ...
    table = Table(
        ...
        caption=(
            f"[dim]DeepEval GEval • Metric: Viral Integrity v1.2 • "
            f"N={len(selected_entries)}[/dim]"
        ),
        show_header=True,
    ...
    with Progress(
        ...
        eval_task = progress.add_task(
            "[yellow]Analyzing performance...", total=len(selected_entries)
        )
        ...
        with Live(table, refresh_per_second=4):
            for entry in selected_entries:
                ...
                try:
                    ...
                    external_score = (
                        VIRAL_METRIC.score if VIRAL_METRIC.score is not None else 0.0
                    )

                    calibration_gap = abs(self_score - external_score)
                    roi = (
                        (external_score * 10) / (cost_usd * 1000)
                        if cost_usd > 0
                        else 0.0
                    )
                    ...
                    console.print(f"[red]Error analyzing entry {request_id}: {e}[/red]")
    ...
    if results:
        ...
        for v in sorted(version_groups.keys()):
            ...
            summary_lines.append(
                f"  └─ MCE: [{mce_color}]{v_mce:.4f}[/{mce_color}] | "
                f"ROI: [{roi_color}]{v_roi:,.1f}x[/{roi_color}]"
            )
        ...
        summary_lines.append(
            "[dim]• ROI: Quality-to-Cost (Judge Score / USD Cost)[/dim]"
        )

        summary_lines.append("─" * 45)
        summary_lines.append(
            f"[bold white]Report:[/bold white] [underline]{REPORT_FILE}[/underline]"
        )
...
def generate_markdown_report(results: list[AnalyticsResult]):
    ...
    with open(REPORT_FILE, "w", encoding="utf-8") as f:
        ...
        for r in results:
            f.write(
                f"| {r.request_id} | {r.prompt_version} | {r.model_name} | "
                f"{r.self_score:.2f} | {r.external_score:.2f} | "
                f"{r.calibration_gap:.4f} | {r.roi:.1f}x |\n"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num", type=int, default=5, help="Number of examples to analyze"
    )
    parser.add_argument(
        "--random", action="store_true", help="Sample randomly from the logs"
    )
    parser.add_argument(
        "--version",
        type=str,
        default=None,
        help="Filter by prompt version (e.g. 1.0.0)",
    )
