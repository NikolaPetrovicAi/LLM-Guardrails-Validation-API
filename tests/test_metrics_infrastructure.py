...
async def test_metrics_logging_integration(temp_metrics_log, pii_service):
    """
    Simulates 3 API calls and verifies that metrics are correctly logged to the JSONL file.
    """
    ...
    responses = [
        ViralScriptResponse(
            ...
            audit=ViralAudit(hook_strength=0.7 + (i * 0.1), retention_reasoning="Good", suggested_edits=[])
        ) for i in range(3)
    ]
    ...
    for i, line in enumerate(lines):
        ...
        print(f"Verified call {i}: cost={data['cost_usd']}, latency={data['latency_ms']}, score={data['self_audit_hook_strength']}")

if __name__ == "__main__":
    import asyncio
    # Simple runner if called directly
    asyncio.run(test_metrics_logging_integration("logs/test_metrics.jsonl", PIIMaskingService()))
