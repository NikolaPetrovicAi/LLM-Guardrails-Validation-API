...
class SemanticCacheService:
        ...
        for key in self.l2_cache:
            if isinstance(key, str) and key.startswith("vec:"):
                ...
                if vector is not None:
                    ...
                    count += 1
        logger.info(
            f"Tiered Semantic Cache loaded with {count} entries into Vector Store."
        )
        ...
        try:
            ...
            if similarity >= threshold:
                ...
                if cached_response:
                    ...
                    self._log_cache_metric(
                        "L2_SEMANTIC_HIT", text, similarity, start_time
                    )
    ...
    def _log_cache_metric(
        self, status: str, text: str, similarity: float, start_time: float
    ):
