...
import httpx
from deepeval.metrics import AnswerRelevancyMetric, FaithfulnessMetric, ToxicityMetric, BiasMetric
...
def run_sync_evaluation(api_outputs: List[Tuple[EvalCase, str]]) -> List[EvalResult]:
    ...
    metrics = {
        "relevancy": AnswerRelevancyMetric(threshold=THRESHOLD, model=EVAL_MODEL, async_mode=False),
        "faithfulness": FaithfulnessMetric(threshold=THRESHOLD, model=EVAL_MODEL, async_mode=False),
        "toxicity": ToxicityMetric(threshold=THRESHOLD, model=EVAL_MODEL, async_mode=False),
    ...
    for case, actual_output in api_outputs:
        ...
        try:
            test_case = LLMTestCase(input=case.input, actual_output=actual_output, retrieval_context=[case.input])
            scores = {}
            for name, metric in metrics.items():
                ...
                time.sleep(1)
            results.append(EvalResult(name=case.name, input=case.input, actual_output=actual_output,
                                      relevancy=scores["relevancy"], faithfulness=scores["faithfulness"],
                                      toxicity=1.0 - scores["toxicity"], bias=1.0 - scores["bias"]))
            time.sleep(2)
        except Exception as e:
            print(f"  ❌ Eval Error: {str(e)}")
            results.append(EvalResult(name=case.name, input=case.input, actual_output=actual_output,
                                      relevancy=0, faithfulness=0, toxicity=0, bias=0, success=False, error=str(e)))
    return results
...
def generate_report(results: List[EvalResult]):
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        if successful_results:
            avg_rel = sum(r.relevancy for r in successful_results) / len(successful_results)
            avg_faith = sum(r.faithfulness for r in successful_results) / len(successful_results)
            avg_tox = sum(r.toxicity for r in successful_results) / len(successful_results)
            avg_bias = sum(r.bias for r in successful_results) / len(successful_results)
        ...
        f.write("| Case Name | Relevancy | Faithfulness | Cleanliness | Fairness | Success |\n")
        f.write("| :--- | :---: | :---: | :---: | :---: | :---: |\n")
        
        for r in results:
            success_icon = "✅" if r.success else "❌"
            f.write(f"| {r.name} | {r.relevancy:.2f} | {r.faithfulness:.2f} | {r.toxicity:.2f} | {r.bias:.2f} | {success_icon} |\n")
